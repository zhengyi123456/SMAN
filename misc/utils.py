...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, mask, caption_model):
        if caption_model == 'all_gru':
            output1 = -input[:, 0, :, :].gather(2, target.unsqueeze(2)).squeeze(2) * mask
            output2 = -input[:, 1, :, :].gather(2, target.unsqueeze(2)).squeeze(2) * mask
            output3 = -input[:, 2, :, :].gather(2, target.unsqueeze(2)).squeeze(2) * mask
